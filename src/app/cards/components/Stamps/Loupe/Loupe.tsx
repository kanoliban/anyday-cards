...
function Loupe({
  ...
  baseScale: number;
  gridCellSize: number;
  activeCardContainerRef: React.RefObject<HTMLElement | null>;
}) {
  ...
  useEffect(() => {
    ...
    const cardImgEl =
      activeCardContainerRef.current?.querySelector(`[data-slot="card-image"]`) ||
      document.querySelector(`[data-id="${selectedCard.id}"] [data-slot="card-image"]`);
    if (!cardImgEl) {
      return;
    }
    ...
    loadImages
      .then(([frontImg, backImg]) => {
        ...
        const imgWidth = cardImgEl.clientWidth * baseScale;
        const imgHeight = cardImgEl.clientHeight * baseScale;
  ...
  return (
    ...
    >
      <div
        ...
        role="region"
        tabIndex={0}
        aria-label="Cards Loupe"
        aria-describedby="cards-loupe-description"
      ...
      <Dial
        ...
        inscription="PEAK 1983  ⎟  10× Measuring Loupe  ⎟  Card Collection · AnyDayCard"
        size={dialSize}
      ...
      />
      <p id="cards-loupe-description" className="sr-only">
